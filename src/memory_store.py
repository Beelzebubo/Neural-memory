...
class VectorMemoryStore:
    ...
    def store(self, id: str, embedding: List[float], metadata: dict):
        ...
        if not self._ids:
            ...
            raise ValueError(
                f"Embedding dimension {dim} does not match expected {self._dim}"
            )
    ...
    def load(self, path: str):
        ...
        if not isinstance(data, dict):
            raise ValueError(f"Invalid store format: expected dict, got {type(data).__name__}")
        self._ids = list(data.get("ids", []))
        raw_embeddings = data.get("embeddings")
        if raw_embeddings is not None and hasattr(raw_embeddings, "ndim") and raw_embeddings.ndim == 2:
            self._embeddings = [raw_embeddings[i] for i in range(len(self._ids))]
        else:
            self._embeddings = []
